    ...
    {recentViewed.slice(0, 3).map((property, i) => (
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm text-green-600">{property?.currency}{property?.price} </p>
          <p className="text-xs text-gray-500">{property?.location} {property?.city} {property?.state}</p>
        </div>
      </div>
      </Link>
    ))}
