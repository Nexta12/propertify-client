...
import { formatTitleCase, getLoggedInUserPath } from "@utils/helper";
...
const DesktopTopbar = () => {
  ...
  return (
    <header className="hidden md:flex bg-white shadow-sm pr-3 justify-between items-center sticky top-0 z-10 ">

       <div className="hidden  p-4 md:flex items-start gap-3">
              <Link to={paths.index}>
                <div className="">
                   <img src={GreenLogo} alt="Logo" width={130} />
                </div>
              </Link>
            </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-2">
          <div className="text-right">
            <p className="text-sm font-medium">{user?.title} {user?.lastName} {user?.firstName}</p>
            <p className="text-xs text-gray-500">{ formatTitleCase(user?.profession)}</p>
          </div>
