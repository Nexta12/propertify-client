...
const Notification = () => {
  ...
  const handleSeen = async (notifId) => {
    try {
      ...
      setNotifications((prevNotifs) =>
        prevNotifs.map((notif) => (notif._id === notifId ? { ...notif, isSeen: true } : notif))
      );
    } catch (error) {
      toast.error(ErrorFormatter(error));
    }
  ...
  const handleDelete = async (notifId) => {
    try {
      ...
      setNotifications((prevNotifs) => prevNotifs.filter((notif) => notif._id !== notifId));
      setMenuOpenId(null);
    } catch (error) {
      toast.error(ErrorFormatter(error));
    }
            ...
            <div className=" min-h-20 max-h-60 overflow-y-auto">
              {notifications?.length > 0 ? (
                notifications.map((notif, index) => (
                  ...
                  >
                    <div onClick={() => handleSeen(notif._id)} className="cursor-pointer flex-1">
                    ...
                    <div className="relative">
                      <button
                        onClick={() =>
                          setMenuOpenId((prev) => (prev === notif._id ? null : notif._id))
                        }
