...
const SingleTicket = () => {
  ...
  const { subject, status, message, priority, createdAt, userId, replies = [] } = ticket;
  ...
  const handleClosed = async () => {
    ...
    try {
      await apiClient.put(`${endpoints.updateTicket}/${id}`, data);
        ...
        {replies.length === 0 ? (
          ...
          <ul className="space-y-4 max-h-96 overflow-y-auto ">
            {replies.map((reply, index) => {
              ...
              const fullName = sender.firstName ? `${sender.firstName} ${sender.lastName}` : "User";
              ...
              return (
                <li
                  ...
                  className={`border-t pt-2 p-3 rounded ${
                    isAdmin ? "bg-orange-50 dark:bg-orange-100" : "bg-gray-50 dark:bg-gray-700"
                  }`}
                >
