...
const Messages = () => {
  ...
  const confirmDelete = async () => {
    ...
    if (itemToDelete) {
      try {
        ...
        setReceivedMessages((prev) => prev.filter((msg) => msg._id !== itemToDelete._id));

        setSentMessages((prev) => prev.filter((msg) => msg._id !== itemToDelete._id));
        setOpenModal(false);
  ...
  const MessageCard = ({ message, isSent = false, onDelete }) => {
    ...
    if (isSent && message.messageType == "direct") {
      ...
      recipientsName =
        message.contacts?.map((item) => `${item.firstName} ${item.lastName}`).join(", ") || "";
    }
      ...
      {activeTab === "received" &&
        (receivedMessages.length > 0 ? (
          receivedMessages.map((msg, i) => (
            <MessageCard key={i} message={msg} onDelete={handleDelete} />
          ))
        ) : (
