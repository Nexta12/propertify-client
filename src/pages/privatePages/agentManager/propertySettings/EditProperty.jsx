...
const EditProperty = () => {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        ...
        setFormData({
          ...propertyData,
          existingImages: propertyData.otherImages || [], // Store original images
          otherImages: propertyData.otherImages || [], // Initialize with existing images
        });
  ...
  const handleFileUpload = (name, files) => {
    setFormData((prev) => {
      ...
      const existingUrls = prev.otherImages.map((img) =>
      ...
      const combined = [...prev.otherImages, ...newFiles].slice(0, 10);
  ...
  const confirmDelete = async () => {
    ...
    if (itemToDelete) {
      try {
        ...
        setFormData((prev) => ({
          ...prev,
          otherImages: prev.otherImages.filter((img) => img !== itemToDelete),
          existingImages: prev.existingImages.filter(
            (img) => img !== itemToDelete
          ),
        }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.data.slug !== slug) {
        navigate(`${paths.agent}/properties/edit/${res.data.data.slug}`, {
          replace: true,
        });
      }
      setTimeout(() => {
  ...
  const handleCancel = async () => {
    if (hasUploadedImages && formData.otherImages.length > 0) {
      setOpenModal(true);
    } else {
  ...
  return (
    <section className="bg-neutral-100 rounded-md max-w-7xl mx-auto p-4">
      <DeleteModal
        isOpen={openModal}
      ...
      />
      <ToastContainer />
      <div className="bg-gradient-to-r from-green-600 to-green-700 px-6 py-5 text-center mb-6 rounded-t-lg">
        <h2 className="text-2xl font-bold text-white">Edit Listing</h2>
      </div>

      <form className="space-y-6 w-full" onSubmit={handleSubmit}>
        {/* Basic Information */}
        <div className="bg-white space-y-6 p-6 rounded-lg ">
        ...
        {/* Amenities */}
        {isResidential && (
          <div className="bg-white space-y-6 rounded-lg ">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <FileUpload
                ...
                onChange={(files) => handleFileUpload("otherImages", files)}
                multiple={true}
                maxFiles={10}
                accept="image/*"
