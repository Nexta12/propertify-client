...
const WelcomeBox = ({ onStarted }) => {
  ...
  const initiateChat = useCallback(
    async (data, autoStart = false) => {
      autoStart ? setInitializing(true) : setLoading(true);

      try {
        const response = await apiClient.post(endpoints.InitiateChatWithAdmin, data);
  ...
  }, [user, onStarted, initiateChat]); // ✅ now safe
  ...
  if (initializing) {
    return (
      <div className="w-full max-w-md mx-auto p-6 bg-white rounded-2xl shadow-lg flex flex-col items-center justify-center">
        ...
        <p className="mt-4 text-gray-600 text-sm">Preparing your chat...</p>
      </div>
    );
  }
