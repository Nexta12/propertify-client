  ...
  return (
    <div className="flex gap-3 p-3 mb-4 rounded-md bg-white dark:bg-gray-800 shadow hover:shadow-md transition cursor-pointer">
      ...
      {property.media.length > 0 && (
        ...
        >
          {isVideo ? (
            <video className="w-full h-full object-cover" src={mediaUrl} muted autoPlay loop />
          ) : (
            <img src={mediaUrl} alt={property.title} className="w-full h-full object-cover" />
          )}
        </Link>
      )}
      {/* Content */}
      <div className="flex flex-col flex-1 justify-between relative">
        {/* Promotion Section */}
        {property.promotionType == "sidebar_widget" && (
          <PromotionBadge post={property} className="text-xs absolute -top-2 right-0 mb-2" />
        )}
        {/* This is used as an ads sample placeholder when publishing an ad */}
        {promoType && (
          <span className="text-xs text-orange absolute -top-2 right-0 mb-2" title="Promoted">
            {promoType}
          </span>
        )}
