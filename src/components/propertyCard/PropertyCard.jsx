...
const PropertyCard = memo(function PropertyCard({
  ...
  return (
    <div
      className={`flex flex-col rounded-sm bg-white dark:bg-gray-900 hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-500 max-w-4xl ${className}`}
    >
      ...
      <div className={`relative w-full group rounded-sm ${leftClass}`}>
        {property?.media?.length > 0 && (
          <Link to={`${paths.properties}/${property.slug}`}>
            {property.media[currentImageIndex].type === "video" ? (
              ...
              >
                <source src={property.media[currentImageIndex].url} type="video/mp4" />
                Your browser does not support the video tag.
              </video>
            ) : (
              <img
                src={property.media[currentImageIndex].url || PropertyPlaceholder}
                alt={property.title}
                className={`w-full h-[250px] sm:h-[350px] md:h-[400px] object-cover ${imgClass}`}
              />
            )}
        ...
        <button
          onClick={prevImage}
          className="absolute left-4 top-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 bg-opacity-80 p-2 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity"
        >
          &larr;
        </button>
        <button
          onClick={nextImage}
          className="absolute right-4 top-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 bg-opacity-80 p-2 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity"
        >
          &rarr;
        </button>
        ...
        {property.promotionType == "featured" && (
          <PromotionBadge
            post={property}
            className="absolute top-4 left-4 py-1 px-2 bg-orange rounded-sm text-white"
          />
        )}

        {/* This help show what ads look like during shooting ads */}
        {promoType && (
          <span className="absolute top-4 left-4 py-1 px-2 text-xs bg-orange rounded-sm text-white">
            {promoType}
          </span>
        )}
      </div>
      ...
      <div className={`w-full p-5 flex flex-col flex-1 ${rightClass}`}>
        ...
        <div className="w-full">
          <Link to={`${paths.properties}/${property.slug}`}>
            <h3 className="font-bold text-gray-900 dark:text-white mt-2 capitalize mb-3">
              {property.title}
            </h3>
          </Link>
          <p className={`text-xs text-gray-700 dark:text-gray-300 ${desClass}`}>
        ...
        <div className="flex items-center text-gray-600 dark:text-gray-400 text-sm mb-4">
          ...
          <span className="line-clamp-1 capitalize text-[12px]">
        ...
        <div className="mt-auto pt-2 border-t border-gray-100 dark:border-gray-700">
          <div className={`flex justify-between items-center relative ${buttonsClass}`}>
